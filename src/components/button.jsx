  ...
  return (
    <>
      {variant === "primary" ? (
        ...
        <button className="max-sm:px-2 max-sm:py-2 px-4 py-3 font-lexend font-medium rounded-full border-2 text-secondary-500 border-secondary-500 hover:border-secondary-600 hover:text-secondary-600 transition-all duration-300">
          {link ? <Link href={url}>{buttonText}</Link> : buttonText}
          {arrowIcon && <ArrowRight />}
        </button>
      ) : (
